...
const SubscriptionSchema = Yup.object().shape({
    ...
    PhoneNo: Yup.string().matches(/[0-9]{4}-[0-9]{7}/, 'Invalid Phone No').required('Required').max(12),
    ...
    DOB: Yup.string(),
    CNIC: Yup.string().matches(/[0-9]{5}-[0-9]{7}-[0-9]{1}/, 'Invalid CNIC No').max(15),
...
const SubscriptionForm = ({ navigation }) => {
    ...
    const mutation = useMutation((values) => userSubscription(values));
    ...
    return (
        <TouchableWithoutFeedback onPress={() => Keyboard.dismiss()}>
            <View style={globalStyles.body}>
                <Formik initialValues={initialValues} validationSchema={SubscriptionSchema} onSubmit={(values, actions) => handleSubmit(values, actions)}>
                    {({ handleChange, handleBlur, handleSubmit, resetForm, values, errors, touched }) => (
                        <View style={{ flex: 1, justifyContent: 'flex-start' }}>
                            <KeyboardAwareScrollView showsVerticalScrollIndicator={false}>
                                <Text style={globalStyles.inputLabel}>Package</Text>
                                <Dropdown value={values.Package} setValue={handleChange('Package')} data={['Package 1', 'Package 2', 'Package 3']} />
                                {errors.Package && touched.Package ? <Text style={globalStyles.ErrorMessages}><ErrorMessage name='Package' /></Text> : <></>}
